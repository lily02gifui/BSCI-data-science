...
void write_out(char* filename, data_t* data, double* input_key, double radius) {
    ...
    if (radius != -1) {
        fprintf(fp, OUTNAME_RADIUS, input_key[0], input_key[1], radius);
    }
    else {
        fprintf(fp, OUTNAME_NEAR, input_key[0], input_key[1]);
    }
    
    fprintf(fp, OUTDATA, data->census_year, data->block_id, data->property_id, 
        data->base_property_id, data->clue_small_area, data->trading_name, data->industry_code, 
        data->industry_description, atof(data->xcoord), atof(data->ycoord), data->location);    
